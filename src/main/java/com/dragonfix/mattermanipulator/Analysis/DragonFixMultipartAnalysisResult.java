...
/**
 * Copies, transforms, restores, and previews ForgeMultipart parts for MatterManipulator builds.
 *
 * <p>
 ...
 *      commit 9d76ed6e</a>
 */
public class DragonFixMultipartAnalysisResult implements ITileAnalysisIntegration, DragonFixMultipartPreviewBridge {
    ...
    public static class PartData {
        ...
        public boolean equals(Object obj) {
            ...
            return java.util.Objects.equals(typeId, other.typeId) && java.util.Objects.equals(nbt, other.nbt)
                && Arrays.equals(drops, other.drops);
        }
    }

    public static DragonFixMultipartAnalysisResult analyze(TileEntity tile) {
        if (!(tile instanceof TileMultipart multipart)) return null;

        List<TMultiPart> partList = multipart.jPartList();
        if (partList.isEmpty()) return null;

        DragonFixMultipartAnalysisResult result = new DragonFixMultipartAnalysisResult();
    ...
    private static PartData capturePart(TMultiPart part) {
        ...
        for (int i = 0; i < dropList.size(); i++) {
            data.drops[i] = PortableItemStack.withNBT(dropList.get(i));
        }
    ...
    private static boolean tryConsumePartItems(IBlockApplyContext ctx, PartData data) {
        ...
        for (PortableItemStack drop : data.drops) {
            ...
            boolean ok = ctx
                .tryConsumeItems(Collections.singletonList(BigItemStack.create(stack)), IPseudoInventory.CONSUME_FUZZY)
                .leftBoolean();
    ...
    private static boolean tryPlacePart(IBlockApplyContext ctx, World world, BlockCoord coord, PartData data) {
        ...
        try {
            ...
            if (result == null) {
                MMMod.LOG.warn("addPart returned null for {} at {},{},{}", data.typeId, coord.x, coord.y, coord.z);
                refundPartItems(ctx, data);
    ...
    private static void refundPartItems(IBlockApplyContext ctx, PartData data) {
        ...
        for (PortableItemStack drop : data.drops) {
            ...
            if (refund != null) ctx.givePlayerItems(refund);
        }
    }
    ...
    private static Block parseMaterialBlock(String materialName) {
        ...
        return i > 0 ? Block.getBlockFromName(materialName.substring(0, i)) : null;
    }

    private static int parseMaterialMeta(String materialName) {
        int i = materialName.lastIndexOf('_');
        if (i > 0 && Block.getBlockFromName(materialName.substring(0, i)) != null) {
            try {
    ...
    public boolean equals(Object obj) {
        if (this == obj) return true;
        if (obj == null || getClass() != obj.getClass()) return false;
        DragonFixMultipartAnalysisResult other = (DragonFixMultipartAnalysisResult) obj;
